      ...
      <div className={`main-container ${isDarkMode ? 'dark' : 'light'}`} style={{ color: theme.text }}>
        ...
        <div style={{ flex: 1, padding: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          <div style={{ position: 'relative', width: '100%', maxWidth: '1100px', aspectRatio: '16/9', backgroundColor: '#000', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5), 0 0 0 1px rgba(255,255,255,0.1)' }}>
            ...
            {activeAsset && activeAsset.type === 'image' && <img ref={activeImgRef} src={`http://localhost:8000/proxy-image?url=${encodeURIComponent(activeAsset.isAlreadyHighRes ? activeAsset.high_res_url : activeAsset.thumb_url)}`} crossOrigin="anonymous" alt="active-b-roll" style={{ display: 'none' }} />}
            {activeAsset && activeAsset.type === 'image' && !activeAsset.isAlreadyHighRes && <img ref={highResImgRef} src={`http://localhost:8000/proxy-image?url=${encodeURIComponent(activeAsset.high_res_url)}`} crossOrigin="anonymous" alt="active-b-roll-highres" onLoad={handleHighResLoad} style={{ display: 'none' }} />}
            {loadingAsset && loadingAsset.type === 'image' && <img src={`http://localhost:8000/proxy-image?url=${encodeURIComponent(warmupCount >= WARMUP_THRESHOLD ? loadingAsset.high_res_url : loadingAsset.thumb_url)}`} crossOrigin="anonymous" alt="loading-b-roll" onLoad={handleImageLoad} onError={handleImageError} style={{ display: 'none' }} />}
